...
class Settings:
    ...
    lakebase_port: int = _env_int("LAKEBASE_PORT", _env_int("PGPORT", 5432, min_value=1), min_value=1)
    ...
    refresh_failure_backoff_minutes: int = _env_int("REFRESH_FAILURE_BACKOFF_MINUTES", 30, min_value=0)
    ...
    max_performance_monitors_per_run: int = _env_int("MAX_PERFORMANCE_MONITORS_PER_RUN", 20, min_value=1)
    ...
    feature_detail_sample_rows_per_day: int = _env_int("FEATURE_DETAIL_SAMPLE_ROWS_PER_DAY", 50000, min_value=1)
