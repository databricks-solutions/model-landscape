...
def test_alpha_selection_returns_figure(reg_data, ridge_estimator):
    _, X, y, _, _ = reg_data
    fig = alpha_selection(
        ridge_estimator, X, y, alphas=np.logspace(-2, 2, 5), cv=3
    )
...
def test_alpha_selection_logs_artifacts(experiment_id, reg_data, ridge_estimator):
    ...
    with mlflow.start_run() as run:
        alpha_selection(
            ridge_estimator, X, y, alphas=np.logspace(-2, 2, 5), cv=3, log=True
        )

    client = mlflow.tracking.MlflowClient()
    json_path = client.download_artifacts(
        run.info.run_id, "lens/panels/alpha_selection.json"
    )
