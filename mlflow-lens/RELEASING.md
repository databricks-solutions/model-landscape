...
2. **Bump the version** in `mlflow-lens/src/mlflow_lens/_version.py` on a
   feature branch:
...
The workflow also runs on any PR that touches `mlflow-lens/` or the
workflow file. That build does everything except the GitHub Release step,
so packaging regressions are caught before merge.
