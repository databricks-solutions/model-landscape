...
from model_landscape import app as app_module
from model_landscape import callbacks as callbacks_module
from model_landscape.app import (
...
def test_validate_workspace_wiring_callback_renders_readiness_card(monkeypatch) -> None:
    monkeypatch.setattr(
        ...
        lambda ready_state, session: {
            ...
            "refresh_workflow_name": "model-landscape-refresh",
...
def test_validate_workspace_wiring_callback_distinguishes_unknown_bootstrap_acl(monkeypatch) -> None:
    monkeypatch.setattr(
        ...
        lambda ready_state, session: {
            ...
            "bootstrap_workflow_name": "model-landscape-bootstrap",
    ...
    class _FakeBackend:
        def get_reference_data(self, model_id):
            ...
            return {
                ...
                "settings": {"refresh_job_id": "", "refresh_job_name": "model-landscape-refresh"},
            }
...
def test_sidebar_status_truncates_long_monitor_description_with_ellipsis_css(monkeypatch) -> None:
    ...
    assert "model-landscape-sidebar-description" in description_component.className
...
def test_sidebar_model_dropdown_uses_sidebar_specific_ellipsis_css() -> None:
    ...
    assert "model-landscape-sidebar-dropdown" in dropdown.className
    assert ".model-landscape-sidebar-dropdown .Select-value-label" in styles.INDEX_STRING
    assert ".model-landscape-sidebar-dropdown .VirtualizedSelectOption" in styles.INDEX_STRING
    assert "text-overflow: ellipsis" in styles.INDEX_STRING
    assert "white-space: nowrap" in styles.INDEX_STRING
