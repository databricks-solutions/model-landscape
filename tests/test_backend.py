...
import model_landscape.backend as backend_module
from model_landscape.backend import (
    DashboardBackend,
    _null_rate_dict,
    _period_label,
    _safe_json_list,
...
from model_landscape.domain.models import BaselinePolicy, InferenceContract, MonitorConfig
from model_landscape.services.refresh_jobs import SharedWorkflowScheduleStatus
from model_landscape.services.thresholds import get_thresholds
...
def _make_backend() -> DashboardBackend:
    config = MonitorConfig(
        ...
        source_table="main.model_landscape_demo.inference_logs",
        ...
        labels_table="main.model_landscape_demo.labels",
    ...
    assert models == [
        {
            ...
            "description": "main.model_landscape_demo.inference_logs | model_id=fraud_model_v1",
            "versions": [],
            "feature_count": 2,
            "slice_columns": ["region"],
            "has_labels": True,
...
def test_get_performance_summary_falls_back_to_source_daily_label_rows_when_persisted_daily_metrics_are_empty() -> (
    ...
    assert performance["timeline"] == [
        {"period": "2026-01-20", "precision": 0.9},
        {"period": "2026-01-21", "precision": 1.0},
    ]
    assert performance["timeline_unavailable_reason"] == ""
...
def test_get_performance_summary_caches_exact_source_daily_label_rows_for_identical_bounds() -> (
    ...
    source_calls: list[tuple[str | None, str | None]] = []
    repository = SimpleNamespace(
        _warehouse=_FakeWarehouse(),
        table_names=SimpleNamespace(
            performance_metrics="performance_metrics",
    ...
    first = backend.get_performance_summary("fraud_model_demo", metric_name="precision")
    second = backend.get_performance_summary("fraud_model_demo", metric_name="precision")
...
def test_get_performance_summary_skips_exact_source_daily_label_rows_when_recent_window_bounds_are_unavailable() -> (
    ...
    class _InvalidWindowWarehouse(_FakeWarehouse):
        def query_params(self, sql: str, params: tuple) -> pd.DataFrame:
            if "FROM performance_metrics" in sql:
                return pd.DataFrame(
                    [
    ...
    repository = SimpleNamespace(
        _warehouse=_InvalidWindowWarehouse(),
        table_names=SimpleNamespace(
            performance_metrics="performance_metrics",
        ),
        list_monitor_configs=lambda status="active": [config],
...
def test_get_latest_window_metrics_fall_back_to_source_daily_label_rows() -> None:
    config = MonitorConfig(
        model_key="fraud_model_demo",
        display_name="Fraud Model Demo",
        source_table="main.model_landscape_demo.inference_logs",
...
def test_get_latest_window_metrics_use_exact_window_bounds_for_source_daily_label_rows() -> None:
    config = MonitorConfig(
        model_key="fraud_model_demo",
        display_name="Fraud Model Demo",
        source_table="main.model_landscape_demo.inference_logs",
...
def test_get_quality_stats_returns_filtered_source_bounds_unavailable_when_no_safe_bounds_exist() -> (
    ...
    repository = SimpleNamespace(
        ...
        get_source_daily_quality_profile_rows=lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("Source fallback should not run without safe bounds")
        ),
    )
    backend = DashboardBackend(repository=repository)
...
def test_get_drift_results_marks_no_filtered_rows_when_bounded_source_probe_is_empty() -> None:
    ...
    repository = SimpleNamespace(
        ...
        get_latest_published_generation_id=lambda model_key: "published-1",
        get_daily_class_feature_profile_rows=lambda *args, **kwargs: [],
        get_source_daily_quality_profile_rows=lambda *args, **kwargs: [],
    )
    backend = DashboardBackend(repository=repository)
    ...
    assert drift.empty
    assert drift.attrs.get("_empty_reason") == "no_filtered_rows"
...
def test_get_overview_rows_uses_bulk_historical_snapshot_queries() -> None:
    ...
    chargeback_config = MonitorConfig(
        ...
        source_table="main.model_landscape_demo.chargebacks",
        contract=InferenceContract(
            timestamp_col="event_ts",
            model_id_col=None,
            prediction_col="prediction",
...
def test_get_reference_data_includes_recent_incident_history_when_available(
    ...
    monkeypatch.setattr(
        ...
        lambda: SharedWorkflowScheduleStatus(
            ...
            resolved=True,
            job_id=123,
            job_name="model-landscape-refresh",
            scheduler_mode="cron",
