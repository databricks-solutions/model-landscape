...
from model_landscape.analytics.performance import compute_bin_edges
from model_landscape.config import settings
from model_landscape.domain.models import MonitorConfig, RefreshResult
...
from model_landscape.services.control_plane import (
...
from model_landscape.services.spark_session import get_spark_session
from model_landscape.services.table_names import TableNames
from model_landscape.services.warehouse import WarehouseConnection
...
def _spark_bin_index_expr(column, *, edges: tuple[float, ...]) -> Any:
    ...
    for bin_index in range(len(edges) - 1):
        ...
        upper_check = (
            numeric <= F.lit(right_edge)
            if bin_index == last_bin_index
            else numeric < F.lit(right_edge)
        )
        condition = (
            numeric.isNotNull() & (~F.isnan(numeric)) & (numeric >= F.lit(left_edge)) & upper_check
        )
    ...
    return (
        expression.when(
            numeric.isNotNull() & (~F.isnan(numeric)) & (numeric < F.lit(first_edge)), F.lit(0)
        )
        .when(
            numeric.isNotNull() & (~F.isnan(numeric)) & (numeric > F.lit(last_edge)),
            F.lit(last_bin_index),
        )
        .otherwise(F.lit(None).cast("int"))
    )
...
def _spark_bucketed_frame(
    ...
    *,
    value_col: str,
    edges: tuple[float, ...],
    output_col: str = "_model_landscape_bin_index",
) -> DataFrame:
    return frame.withColumn(
        output_col, _spark_bin_index_expr(F.col(value_col), edges=edges)
    ...
    *,
    ...
    for index, row in enumerate(rows):
        ...
        if current_value is None:
            raise ValueError(f"{row_kind} row at index {index} is missing {field_name}")
...
class SparkRefreshRepository(ControlPlaneRepository):
    ...
    def _typed_df_from_rows(
        ...
        *,
        ...
        requires_model_key = any(
            field.name == "model_key" and not field.nullable for field in schema.fields
        )
        ...
        requires_window_id = any(
            field.name == "window_id" and not field.nullable for field in schema.fields
        )
        ...
        requires_computed_at = any(
            field.name == "computed_at" and not field.nullable for field in schema.fields
        )
    ...
    def _base_source_df(
        ...
        *,
        ...
        if config.model_version_value and config.contract.model_version_col:
            df = df.filter(
                _spark_col(config.contract.model_version_col) == F.lit(config.model_version_value)
            )
    ...
    def load_source_df(
        ...
        *,
        ...
        if config.labels_order_col:
            rank_window = Window.partitionBy(_spark_col(config.labels_join_col)).orderBy(
                _spark_col(config.labels_order_col).desc()
            )
            labels_df = (
                labels_df.withColumn(
                    "_model_landscape_label_rank", F.row_number().over(rank_window)
                )
                .filter(F.col("_model_landscape_label_rank") == 1)
                .drop("_model_landscape_label_rank")
            )
        labels_df = labels_df.select(
            _spark_col(config.labels_join_col).alias("_model_landscape_join_key"),
            _spark_col(config.contract.label_col).alias(config.contract.label_col),
        )
        return df.join(
            labels_df,
            df[source_join_col] == labels_df["_model_landscape_join_key"],
            "left",
        ).drop("_model_landscape_join_key")
    ...
    def get_source_date_range(
        ...
        *,
        ...
        df, _ = self._base_source_df(
            config, start_date=start_date, end_date=end_date, selected_columns=()
        )
        row = df.agg(
            F.date_format(F.min(_spark_col(config.contract.timestamp_col)), "yyyy-MM-dd").alias(
                "min_date"
            ),
            F.date_format(F.max(_spark_col(config.contract.timestamp_col)), "yyyy-MM-dd").alias(
                "max_date"
            ),
        ).first()
    ...
    def get_source_profile(
        ...
        *,
        ...
        summary = df.agg(
            ...
            (
                F.sum(
                    F.when(_spark_col(config.contract.label_col).isNotNull(), F.lit(1)).otherwise(
                        F.lit(0)
                    )
                )
    ...
    def get_label_watermark(
        ...
        *,
        ...
        if config.labels_table and config.labels_order_col:
            row = (
                self._read_table(config.labels_table)
                .agg(F.max(_spark_col(config.labels_order_col)).cast("string").alias("watermark"))
                .first()
            )
            ...
            try:
                rows = list(
                    _iter_local_rows(
                        ...
                        .select(
                            F.date_format(F.col("baseline_start"), "yyyy-MM-dd").alias(
                                "baseline_start"
                            ),
                            F.date_format(F.col("baseline_end"), "yyyy-MM-dd").alias(
                                "baseline_end"
                            ),
                            F.date_format(F.col("window_start"), "yyyy-MM-dd").alias(
                                "window_start"
                            ),
                            F.date_format(F.col("window_end"), "yyyy-MM-dd").alias("window_end"),
                        )
    ...
    def _generation_scoped_tables(
        self, *, include_quality_metrics: bool = False
    ) -> tuple[str, ...]:
        tables = [
    ...
    def _copy_generation_state(
        ...
        *,
        ...
        for table_name in self._generation_scoped_tables():
            frame = self._read_table(table_name).filter(
                (F.col("model_key") == F.lit(model_key))
    ...
    def replace_performance_bin_specs(
        ...
        *,
        ...
        predicate = f"model_key = {_sql_string_literal(model_key)}" + (
    ...
    def get_current_incident_state(
        self, model_key: str
    ...
    def build_daily_profiles(
        ...
        *,
        ...
        df = (
            ...
            .withColumn(
                "_model_landscape_profile_date",
                F.to_date(_spark_col(config.contract.timestamp_col)),
            )
            .filter(F.col("_model_landscape_profile_date").isNotNull())
        )
        df = df.persist()
        try:
            numeric_bin_specs = (
            ...
            daily_quality_rows = (
                self._build_daily_quality_profile_rows(
                    config=config, source_df=df, computed_at=computed_at
                )
                if include_drift_quality
                else []
            )
            daily_class_quality_rows = (
                self._build_daily_class_quality_profile_rows(
                    config=config, source_df=df, computed_at=computed_at
                )
                if include_drift_quality
                else []
    ...
    def _metadata_df(
        self, metadata_list: list[dict[str, str]], *, default_model_key: str | None = None
    ) -> DataFrame:
        frame = self._typed_df_from_rows(
            metadata_list,
        ...
        return (
            frame.withColumn("window_start", F.to_date(F.col("window_start")))
    ...
    def _rewrite_quality_summary(self, model_key: str, *, source_run_id: str | None = None) -> None:
        ...
        quality_df = self._read_table(self._table_names.daily_quality_profiles).filter(
            F.col("model_key") == F.lit(model_key)
        )
        if generation_id:
            quality_df = quality_df.filter(F.col("source_run_id") == F.lit(generation_id))
        quality_df = quality_df.withColumn(
            "_null_rates_map", F.from_json(F.col("null_rates"), _NULL_RATE_SCHEMA)
        )
        if not quality_df.take(1):
            return
        aggregate = quality_df.agg(
        ...
        variance_numerator = max(
            float(aggregate["_variance_terms"] or 0.0) - (total_rows * (prediction_mean**2)), 0.0
        )
        ...
        daily_volume = {
            ...
            for row in _iter_local_rows(
                quality_df.select(
                    F.date_format(F.col("profile_date"), "yyyy-MM-dd").alias("profile_date"),
                    "row_count",
                ).orderBy("profile_date")
            )
            if row["profile_date"] is not None
        ...
        null_rate_rows = (
            quality_df.select(
                "row_count",
                F.explode_outer(F.map_entries(F.col("_null_rates_map"))).alias("_entry"),
            )
            .select(
                "row_count",
        ...
        summary_rows = [
            {
                "model_key": model_key,
                "total_rows": total_rows,
                "min_date": str(aggregate["min_date"])
                if aggregate["min_date"] is not None
                else None,
                "max_date": str(aggregate["max_date"])
                if aggregate["max_date"] is not None
                else None,
    ...
    def replace_all_refresh_results(
        self, model_key: str, result: RefreshResult, source_run_id: str | None = None
    ) -> None:
        generation_id = source_run_id or ""
        self._append_df_to_table(
            self._table_names.comparison_windows,
            self._window_df_from_rows(
                result.window_rows, source_run_id=source_run_id, default_model_key=model_key
            ),
        )
        self._append_df_to_table(
            self._table_names.drift_metrics,
            self._drift_df_from_rows(
                result.drift_rows, source_run_id=generation_id, default_model_key=model_key
            ),
        )
        self._append_df_to_table(
            self._table_names.quality_history,
        ...
        self.replace_performance_bin_specs(
            model_key, result.performance_bin_specs, source_run_id=generation_id
        )
        self._append_df_to_table(
            self._table_names.incidents,
            self._incident_df_from_rows(
                result.incident_rows, source_run_id=generation_id, default_model_key=model_key
            ),
        )
        self._append_df_to_table(
            self._table_names.incident_history,
    ...
    def append_refresh_result(
        ...
        incident_history_windows = {
            str(row.get("window_id") or "") for row in result.incident_history_rows
        }
        daily_quality_dates = {
            str(row.get("profile_date") or "") for row in result.daily_quality_profile_rows
        }
        ...
        daily_feature_dates = {
            str(row.get("profile_date") or "") for row in result.daily_feature_profile_rows
        }
        ...
        daily_performance_dates = {
            str(row.get("profile_date") or "") for row in result.daily_performance_profile_rows
        }
        daily_label_metric_dates = {
            str(row.get("profile_date") or "") for row in result.daily_label_metric_rows
        }
        ...
        _delete_string_values(
            self._table_names.incident_history, "window_id", incident_history_windows
        )
        _delete_date_values(
            self._table_names.daily_quality_profiles, "profile_date", daily_quality_dates
        )
        ...
        _delete_date_values(
            self._table_names.daily_feature_profiles, "profile_date", daily_feature_dates
        )
        ...
        _delete_date_values(
            self._table_names.daily_performance_profiles, "profile_date", daily_performance_dates
        )
        _delete_date_values(
            self._table_names.daily_label_metrics, "profile_date", daily_label_metric_dates
        )
        ...
        self._append_df_to_table(
            self._table_names.comparison_windows,
            self._window_df_from_rows(
                result.window_rows, source_run_id=generation_id, default_model_key=model_key
            ),
        )
        self._append_df_to_table(
            self._table_names.drift_metrics,
        ...
        if result.performance_bin_specs:
            ...
            self.replace_performance_bin_specs(
                model_key, persisted_specs, source_run_id=generation_id
            )
        self._append_df_to_table(
            self._table_names.incidents,
            self._incident_df_from_rows(
    ...
    def _derive_performance_rows_from_df(
        ...
        *,
        ...
        joined = (
            ...
            .withColumn(
                "baseline_metric", F.col("baseline_weighted_sum") / F.col("baseline_row_count")
            )
            .withColumn(
                "current_metric", F.col("current_weighted_sum") / F.col("current_row_count")
            )
            .withColumn(
                "delta",
                (
            ...
            .withColumn(
                "_total_current_rows",
                F.greatest(
                    F.coalesce(F.col("total_current_rows"), F.col("current_row_count")), F.lit(1)
                ),
            )
            .withColumn(
                "volume_pct",
                (F.col("current_row_count") / F.col("_total_current_rows")) * F.lit(100.0),
            )
    ...
    def _derive_categorical_drift_rows_from_df(
        ...
        *,
        ...
        if not categorical_features:
            return []
        categorical_df = (
            feature_df.filter(F.col("feature_kind") == F.lit("categorical"))
        ...
        counts = (
            counts.withColumn("_ref_total", F.sum("ref_count").over(partition))
            .withColumn("_cur_total", F.sum("cur_count").over(partition))
            .filter((F.col("_ref_total") > 0) & (F.col("_cur_total") > 0))
            .withColumn(
                "_ref_prop_raw", (F.col("ref_count") / F.col("_ref_total")) + F.lit(EPSILON)
            )
            .withColumn(
                "_cur_prop_raw", (F.col("cur_count") / F.col("_cur_total")) + F.lit(EPSILON)
            )
            ...
            .withColumn(
                "psi_component",
                (F.col("cur_prop") - F.col("ref_prop"))
                * F.log(F.col("cur_prop") / F.col("ref_prop")),
            )
            .withColumn(
                "kl_component", F.col("cur_prop") * F.log(F.col("cur_prop") / F.col("ref_prop"))
            )
            .withColumn(
                "js_component",
                F.lit(0.5)
        ...
        for row in _iter_local_rows(metrics):
            ...
            common_payload = {
                ...
                "ref_null_pct": round(
                    float(
                        (ref_total_rows - int(row["ref_non_null_count"] or 0))
                        / ref_total_rows
                        * 100
                    ),
                    2,
                ...
                "cur_null_pct": round(
                    float(
                        (cur_total_rows - int(row["cur_non_null_count"] or 0))
                        / cur_total_rows
                        * 100
                    ),
                    2,
    ...
    def _derive_numeric_drift_rows_from_df(
        ...
        *,
        ...
        numeric_features = tuple(
            feature for feature in config.contract.feature_columns if feature not in categorical_set
        )
        if not numeric_features:
            return []
        empty_array = F.array().cast(ArrayType(DoubleType()))
        numeric_df = (
            feature_df.filter(F.col("feature_kind") == F.lit("numeric"))
        ...
        def _aggregate_numeric_range(
            date_start_col: str, date_end_col: str, prefix: str
            ...
            stats = joined.groupBy(*window_cols, "feature_name").agg(
            ...
            counts = (
                joined.select(
                    *window_cols,
                    ...
                    F.element_at(F.col("_hist_edges"), F.col("bin_index") + F.lit(1)).alias(
                        "bin_left"
                    ),
                    F.element_at(F.col("_hist_edges"), F.col("bin_index") + F.lit(2)).alias(
                        "bin_right"
                    ),
                )
        ...
        baseline_stats, baseline_bins = _aggregate_numeric_range(
            "baseline_start", "baseline_end", "ref"
        )
        ...
        counts = baseline_bins.join(current_bins, on=count_join_columns, how="full_outer").select(
            *[
                F.coalesce(
        ...
        partition = Window.partitionBy(*window_cols, "feature_name")
        joined = (
            counts.withColumn("_ref_total", F.sum("ref_bin_count").over(partition))
            .withColumn("_cur_total", F.sum("cur_bin_count").over(partition))
            .filter((F.col("_ref_total") > 0) & (F.col("_cur_total") > 0))
            .withColumn(
                "_ref_prop_raw", (F.col("ref_bin_count") / F.col("_ref_total")) + F.lit(EPSILON)
            )
            .withColumn(
                "_cur_prop_raw", (F.col("cur_bin_count") / F.col("_cur_total")) + F.lit(EPSILON)
            )
            .withColumn("_ref_prop_denom", F.sum("_ref_prop_raw").over(partition))
            .withColumn("_cur_prop_denom", F.sum("_cur_prop_raw").over(partition))
            .withColumn("ref_prop", F.col("_ref_prop_raw") / F.col("_ref_prop_denom"))
        ...
        metrics = (
            ...
            .withColumn(
                ...
                F.greatest(
                    F.col("ref_variance_terms")
                    - (F.col("ref_non_null_count") * F.pow(F.col("ref_mean"), 2)),
                    F.lit(0.0),
                ),
            )
            .withColumn(
                ...
                F.greatest(
                    F.col("cur_variance_terms")
                    - (F.col("cur_non_null_count") * F.pow(F.col("cur_mean"), 2)),
                    F.lit(0.0),
                ),
            )
            .withColumn(
                "ref_std", F.sqrt(F.col("_ref_variance_numerator") / F.col("ref_non_null_count"))
            )
            .withColumn(
                "cur_std", F.sqrt(F.col("_cur_variance_numerator") / F.col("cur_non_null_count"))
            )
            .withColumn(
                "ref_null_pct",
                F.when(
                    ...
                    (
                        (F.col("ref_total_rows") - F.col("ref_non_null_count"))
                        / F.col("ref_total_rows")
                    )
                    * F.lit(100.0),
                ).otherwise(F.lit(0.0)),
            )
            .withColumn(
                ...
                F.when(
                    ...
                    (
                        (F.col("cur_total_rows") - F.col("cur_non_null_count"))
                        / F.col("cur_total_rows")
                    )
                    * F.lit(100.0),
                ).otherwise(F.lit(0.0)),
            )
        ...
        if derivation_start and derivation_end:
            if include_drift_quality:
                quality_df = self._merge_profile_df(
                    persisted_df=self._load_persisted_daily_quality_profile_df(
                        config.model_key, derivation_start, derivation_end
                    ),
                    current_df=current_quality_df,
                    key_columns=("profile_date",),
                )
                feature_df = self._merge_profile_df(
                    persisted_df=self._load_persisted_daily_feature_profile_df(
                        config.model_key, derivation_start, derivation_end
                    ),
            ...
            if include_performance:
                performance_df = self._merge_profile_df(
                    persisted_df=self._load_persisted_daily_performance_profile_df(
                        config.model_key, derivation_start, derivation_end
                    ),
        ...
        incident_rows = (
            build_incidents(
                [
                    ...
                    if str(row.get("window_end") or "") == latest_window_end
                ],
    ...
    def _build_daily_quality_profile_rows(
        ...
        *,
        ...
        null_rate_features = [
            feature for feature in config.contract.feature_columns if feature in source_df.columns
        ]
        agg_exprs.extend(
            F.round(
                F.avg(F.when(_spark_col(feature).isNull(), F.lit(100.0)).otherwise(F.lit(0.0))),
        ...
        rows = (
            source_df.groupBy("_model_landscape_profile_date")
            .agg(*agg_exprs)
            .orderBy("_model_landscape_profile_date")
        )
        ...
        for row in _iter_local_rows(rows):
            ...
            payload.append(
                {
                    "model_key": config.model_key,
                    "profile_date": str(row["_model_landscape_profile_date"]),
                    "row_count": int(row["row_count"] or 0),
                    "prediction_mean": _safe_float(row["prediction_mean"]),
                    "prediction_std": _safe_float(row["prediction_std"]),
                    "null_rates": json.dumps(null_rates),
    ...
    def _build_daily_numeric_feature_stats_map(
        ...
        *,
        ...
        if not features:
            return {}
        agg_exprs: list[Any] = [
            F.count("*").alias("_model_landscape_row_count"),
        ]
        alias_map: dict[str, dict[str, str]] = {}
        for index, feature in enumerate(features):
            value_col = _spark_col(feature).cast("double")
            aliases = {
                "non_null_count": f"_model_landscape_num_{index}_non_null_count",
                "null_pct": f"_model_landscape_num_{index}_null_pct",
                "mean": f"_model_landscape_num_{index}_mean",
                "std": f"_model_landscape_num_{index}_std",
                "min_value": f"_model_landscape_num_{index}_min_value",
                "max_value": f"_model_landscape_num_{index}_max_value",
            }
            alias_map[feature] = aliases
            agg_exprs.extend(
                [
                    F.sum(F.when(value_col.isNotNull(), F.lit(1)).otherwise(F.lit(0))).alias(
                        aliases["non_null_count"]
                    ),
                    F.round(
        ...
        for row in _iter_local_rows(rows):
            profile_date = str(row["_model_landscape_profile_date"])
            row_count = int(row["_model_landscape_row_count"] or 0)
    ...
    def _build_daily_categorical_feature_stats_map(
        ...
        *,
        ...
        for index, feature in enumerate(features):
            aliases = {
                "non_null_count": f"_model_landscape_cat_{index}_non_null_count",
                "null_pct": f"_model_landscape_cat_{index}_null_pct",
            }
            alias_map[feature] = aliases
            agg_exprs.extend(
                [
                    F.sum(
                        F.when(_spark_col(feature).isNotNull(), F.lit(1)).otherwise(F.lit(0))
                    ).alias(aliases["non_null_count"]),
                    F.round(
                        F.avg(
                            F.when(_spark_col(feature).isNull(), F.lit(100.0)).otherwise(F.lit(0.0))
                        ),
                        2,
                    ).alias(aliases["null_pct"]),
                ]
    ...
    def _build_daily_numeric_feature_rows(
        ...
        *,
        ...
        numeric = source_df.withColumn(
            "_model_landscape_numeric_value", _spark_col(feature).cast("double")
        )
        ...
        if len(edges) >= 2:
            count_rows = (
                _spark_bucketed_frame(
                    numeric.filter(F.col("_model_landscape_numeric_value").isNotNull()),
                    value_col="_model_landscape_numeric_value",
                    edges=edges,
                )
                .groupBy("_model_landscape_profile_date", "_model_landscape_bin_index")
                .agg(F.count("*").alias("bin_count"))
                .orderBy("_model_landscape_profile_date", "_model_landscape_bin_index")
            )
            for row in _iter_local_rows(count_rows):
                key = str(row["_model_landscape_profile_date"])
                counts = count_map.setdefault(key, [0.0] * (len(edges) - 1))
                bin_index = int(row["_model_landscape_bin_index"] or 0)
        ...
        if not feature_stats:
            stats_rows = (
                numeric.groupBy("_model_landscape_profile_date")
                .agg(
                    F.count("*").alias("row_count"),
                    F.sum(
                        F.when(
                            F.col("_model_landscape_numeric_value").isNotNull(), F.lit(1)
                        ).otherwise(F.lit(0))
                    ).alias("non_null_count"),
                    F.round(
                        F.avg(
                            F.when(
                                F.col("_model_landscape_numeric_value").isNull(), F.lit(100.0)
                            ).otherwise(F.lit(0.0))
                        ),
                        2,
                    ).alias("null_pct"),
                    F.avg("_model_landscape_numeric_value").alias("mean"),
                    F.stddev_samp("_model_landscape_numeric_value").alias("std"),
                    F.min("_model_landscape_numeric_value").alias("min_value"),
                    F.max("_model_landscape_numeric_value").alias("max_value"),
                )
                .orderBy("_model_landscape_profile_date")
            )
            feature_stats = {
                str(row["_model_landscape_profile_date"]): {
                    "row_count": int(row["row_count"] or 0),
        ...
        for profile_date in sorted(feature_stats):
            ...
            distribution_payload = {
                ...
                "counts": [
                    round(float(value), 6)
                    for value in count_map.get(profile_date, [0.0] * max(len(edges) - 1, 0))
                ],
                "sample_values": [],
            }
            payload.append(
    ...
    def _build_daily_categorical_feature_rows(
        ...
        *,
        ...
        feature_value = F.coalesce(_spark_col(feature).cast("string"), F.lit("__NULL__")).alias(
            "_model_landscape_feature_value"
        )
        counts_df = (
            source_df.select("_model_landscape_profile_date", feature_value)
            .groupBy("_model_landscape_profile_date", "_model_landscape_feature_value")
            .agg(F.count("*").alias("category_count"))
        )
        rank_window = Window.partitionBy("_model_landscape_profile_date").orderBy(
            F.col("category_count").desc(),
            F.col("_model_landscape_feature_value").asc(),
        )
        top_counts_rows = (
            counts_df.withColumn("_model_landscape_category_rank", F.row_number().over(rank_window))
            .withColumn(
                "_model_landscape_bucket",
                F.when(
                    F.col("_model_landscape_category_rank") <= F.lit(CATEGORICAL_TOP_N),
                    F.col("_model_landscape_feature_value"),
                ).otherwise(F.lit("__OTHER__")),
            )
            .groupBy("_model_landscape_profile_date", "_model_landscape_bucket")
            .agg(F.sum("category_count").alias("bucket_count"))
            .orderBy("_model_landscape_profile_date", "_model_landscape_bucket")
        )
        distribution_map: dict[str, dict[str, int]] = {}
        for row in _iter_local_rows(top_counts_rows):
            profile_date = str(row["_model_landscape_profile_date"])
            distribution_map.setdefault(profile_date, {})[str(row["_model_landscape_bucket"])] = (
                int(row["bucket_count"] or 0)
            )
        payload: list[dict[str, Any]] = []
        feature_stats = stats_by_date or {}
        if not feature_stats:
    ...
    def _build_performance_bin_specs(
        ...
        *,
        ...
        candidate_features = [
            ...
            and feature not in specs
            and feature not in categorical_set
        ]
        ...
        for index, feature in enumerate(candidate_features):
            value_col = _spark_col(feature).cast("double")
            count_alias = f"_model_landscape_{index}_non_null_count"
            min_alias = f"_model_landscape_{index}_min_value"
            max_alias = f"_model_landscape_{index}_max_value"
        ...
        for feature in candidate_features:
            ...
            if (
                non_null_count < max(2, PERFORMANCE_BIN_COUNT)
                or min_value is None
                or max_value is None
            ):
                continue
            numeric_frame = source_df.select(
                _spark_col(feature).cast("double").alias("_value")
            ...
            if config.performance_binning_mode == "quantile":
                ...
                quantiles = np.linspace(
                    clip_fraction, 1.0 - clip_fraction, num=PERFORMANCE_BIN_COUNT + 1, dtype=float
                )
                edges = np.unique(
                    np.asarray(
                        numeric_frame.approxQuantile("_value", quantiles.tolist(), 0.001),
                        dtype=float,
                    )
                )
                if len(edges) < 2:
                ...
                if config.performance_binning_clip_percentile:
                    ...
                    bounds = numeric_frame.approxQuantile(
                        "_value", [clip_fraction, 1.0 - clip_fraction], 0.001
                    )
                    if len(bounds) == 2:
    ...
    def _build_daily_performance_profile_rows(
        ...
        *,
        ...
        total_rows_df = source_df.groupBy("_model_landscape_profile_date").agg(
        ...
        for feature, edges in sorted(bin_specs.items()):
            if feature not in source_df.columns:
                continue
            metric_frame = (
                self._daily_regression_metrics_by_bin_df(
                    config, source_df, feature, edges, selected_metric_names
                )
                if regression_mode
                else self._daily_classification_metrics_by_bin_df(
                    config, source_df, feature, edges, selected_metric_names
                )
            )
        ...
        if combined is None:
            return []
        final_frame = (
            combined.join(total_rows_df, on="_model_landscape_profile_date", how="left")
            .withColumn(
                "_total_rows",
                F.greatest(F.coalesce(F.col("_total_rows"), F.col("row_count")), F.lit(1)),
            )
            .withColumn(
                "volume_pct", F.round((F.col("row_count") / F.col("_total_rows")) * F.lit(100.0), 2)
            )
            .select(
                "model_key",
                F.date_format(F.col("_model_landscape_profile_date"), "yyyy-MM-dd").alias(
                    "profile_date"
                ),
                "feature_name",
                "bin_label",
    ...
    def _build_daily_label_metric_rows(
        ...
        *,
        ...
        metrics_df = (
            source_df.select(
                "_model_landscape_profile_date",
                prediction.alias("_model_landscape_prediction"),
                label.alias("_model_landscape_label"),
            )
            .filter(
                F.col("_model_landscape_prediction").isNotNull()
                & F.col("_model_landscape_label").isNotNull()
            )
            .withColumn(
                "_model_landscape_pred_binary", F.col("_model_landscape_prediction").cast("int")
            )
            .withColumn(
                "_model_landscape_truth_binary", F.col("_model_landscape_label").cast("int")
            )
            .groupBy("_model_landscape_profile_date")
            .agg(
                F.sum(
                    F.when(F.col("_model_landscape_truth_binary") == 1, F.lit(1)).otherwise(
                        F.lit(0)
                    )
                ).alias("actual_positive_count"),
                F.sum(
                    F.when(F.col("_model_landscape_truth_binary") == 0, F.lit(1)).otherwise(
                        F.lit(0)
                    )
                ).alias("actual_negative_count"),
                F.sum(
                    F.when(F.col("_model_landscape_pred_binary") == 1, F.lit(1)).otherwise(F.lit(0))
                ).alias("predicted_positive_count"),
                F.sum(
                    F.when(F.col("_model_landscape_pred_binary") == 0, F.lit(1)).otherwise(F.lit(0))
                ).alias("predicted_negative_count"),
                F.sum(
                    F.when(
                        (F.col("_model_landscape_pred_binary") == 1)
                        & (F.col("_model_landscape_truth_binary") == 1),
                        F.lit(1),
                    ).otherwise(F.lit(0))
                ).alias("tp"),
                F.sum(
                    F.when(
                        (F.col("_model_landscape_pred_binary") == 1)
                        & (F.col("_model_landscape_truth_binary") == 0),
                        F.lit(1),
                    ).otherwise(F.lit(0))
                ).alias("fp"),
                F.sum(
                    F.when(
                        (F.col("_model_landscape_pred_binary") == 0)
                        & (F.col("_model_landscape_truth_binary") == 1),
                        F.lit(1),
                    ).otherwise(F.lit(0))
                ).alias("fn"),
            ...
            .withColumn(
                "precision",
                F.when(
                    F.col("tp") + F.col("fp") > 0, F.col("tp") / (F.col("tp") + F.col("fp"))
            ...
            .withColumn(
                ...
                F.when(
                    F.col("tp") + F.col("fp") > 0,
                    F.when(
                        F.col("tp") + F.col("fn") > 0, F.col("tp") / (F.col("tp") + F.col("fn"))
                    ).otherwise(F.lit(None).cast("double")),
                ).otherwise(F.lit(None).cast("double")),
            )
            .withColumn(
                ...
                F.when(
                    ...
                    (F.lit(2.0) * F.col("precision") * F.col("recall"))
            ...
            .withColumn(
                ...
                F.when(
                    ...
                    (F.col("tp") + F.col("tn"))
                    / (F.col("tp") + F.col("fp") + F.col("fn") + F.col("tn")),
                ).otherwise(F.lit(None).cast("double")),
            )
            .orderBy("_model_landscape_profile_date")
        )
    ...
    def _bin_label_expr(
        self, edges: tuple[float, ...], *, bin_col: str = "_model_landscape_bin_index"
    ):
        mapping: list[Any] = []
    ...
    def _daily_classification_metrics_by_bin_df(
        ...
        feature_df = source_df.select(
            "_model_landscape_profile_date",
            _spark_col(feature).cast("double").alias("_model_landscape_feature_value"),
            _spark_binary_prediction_column(source_df, config=config).alias(
                "_model_landscape_prediction"
            ),
            _spark_binary_label_column(config).alias("_model_landscape_label"),
        ).filter(
            F.col("_model_landscape_feature_value").isNotNull()
            & F.col("_model_landscape_prediction").isNotNull()
            & F.col("_model_landscape_label").isNotNull()
        )
        ...
        bucketed = (
            _spark_bucketed_frame(
                feature_df, value_col="_model_landscape_feature_value", edges=edges
            )
            .withColumn(
                "_model_landscape_pred_binary", F.col("_model_landscape_prediction").cast("int")
            )
        ...
        metric_df = (
            bucketed.groupBy("_model_landscape_profile_date", "_model_landscape_bin_index")
            .agg(
                F.count("*").alias("row_count"),
                F.sum(
                    F.when(
            ...
            .withColumn(
                "_model_landscape_bin_index", F.col("_model_landscape_bin_index").cast("int")
            )
        )
        selected = [
            metric_name
            for metric_name in metric_names
            if metric_name in {"f1", "precision", "recall", "accuracy"}
        ]
        if not selected:
            return None
    ...
    def _daily_regression_metrics_by_bin_df(
        ...
        feature_df = source_df.select(
            "_model_landscape_profile_date",
            _spark_col(feature).cast("double").alias("_model_landscape_feature_value"),
            _spark_col(config.contract.prediction_col)
            .cast("double")
            .alias("_model_landscape_prediction"),
            _spark_col(config.contract.label_col).cast("double").alias("_model_landscape_label"),
        ).filter(
            F.col("_model_landscape_feature_value").isNotNull()
            & F.col("_model_landscape_prediction").isNotNull()
            & F.col("_model_landscape_label").isNotNull()
        ...
        metric_df = (
            ...
            .withColumn(
                "_model_landscape_abs_error",
                F.abs(F.col("_model_landscape_prediction") - F.col("_model_landscape_label")),
            )
            .withColumn(
                "_model_landscape_squared_error",
                F.pow(F.col("_model_landscape_prediction") - F.col("_model_landscape_label"), 2),
            )
            .groupBy("_model_landscape_profile_date", "_model_landscape_bin_index")
            .agg(
                F.count("*").alias("row_count"),
                F.avg("_model_landscape_abs_error").alias("mae"),
                F.sqrt(F.avg("_model_landscape_squared_error")).alias("rmse"),
            )
            .withColumn(
                "_model_landscape_bin_index", F.col("_model_landscape_bin_index").cast("int")
            )
...
def build_refresh_repository(
    *,
    ...
    from model_landscape.services.control_plane import _build_read_model
