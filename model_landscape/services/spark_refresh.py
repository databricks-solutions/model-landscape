...
from model_landscape.services.class_filters import NEGATIVE_CLASS_TOKENS, POSITIVE_CLASS_TOKENS, supports_binary_class_filters
...
class SparkRefreshRepository(ControlPlaneRepository):
    ...
    def load_source_df(
        ...
        *,
        ...
        if config.labels_order_col:
            ...
            labels_df = (
                labels_df
                .withColumn("_model_landscape_label_rank", F.row_number().over(rank_window))
    ...
    def build_daily_profiles(
        ...
        *,
        ...
        df = (
            ...
            .withColumn("_model_landscape_profile_date", F.to_date(_spark_col(config.contract.timestamp_col)))
    ...
    def _build_daily_numeric_feature_rows(
        ...
        *,
        ...
        numeric = source_df.withColumn("_model_landscape_numeric_value", _spark_col(feature).cast("double"))
        ...
        if not feature_stats:
            stats_rows = (
                ...
                .agg(
                    F.count("*").alias("row_count"),
                    F.sum(F.when(F.col("_model_landscape_numeric_value").isNotNull(), F.lit(1)).otherwise(F.lit(0))).alias("non_null_count"),
                    F.round(
                        F.avg(F.when(F.col("_model_landscape_numeric_value").isNull(), F.lit(100.0)).otherwise(F.lit(0.0))),
                        2,
                    ).alias("null_pct"),
    ...
    def _build_daily_categorical_feature_rows(
        ...
        *,
        ...
        feature_value = F.coalesce(_spark_col(feature).cast("string"), F.lit("__NULL__")).alias("_model_landscape_feature_value")
        counts_df = (
            source_df
            .select("_model_landscape_profile_date", feature_value)
        ...
        top_counts_rows = (
            counts_df
            .withColumn("_model_landscape_category_rank", F.row_number().over(rank_window))
            .withColumn(
                "_model_landscape_bucket",
                F.when(F.col("_model_landscape_category_rank") <= F.lit(CATEGORICAL_TOP_N), F.col("_model_landscape_feature_value"))
        ...
        for row in _iter_local_rows(top_counts_rows):
            profile_date = str(row["_model_landscape_profile_date"])
            distribution_map.setdefault(profile_date, {})[str(row["_model_landscape_bucket"])] = int(row["bucket_count"] or 0)
        payload: list[dict[str, Any]] = []
        feature_stats = stats_by_date or {}
        if not feature_stats:
            stats_rows = (
    ...
    def _build_daily_performance_profile_rows(
        ...
        *,
        ...
        total_rows_df = source_df.groupBy("_model_landscape_profile_date").agg(F.count("*").alias("_total_rows"))
        regression_mode = (config.problem_type or "classification").strip().lower() == "regression"
        ...
        final_frame = (
            ...
            .select(
                "model_key",
                F.date_format(F.col("_model_landscape_profile_date"), "yyyy-MM-dd").alias("profile_date"),
                "feature_name",
                "bin_label",
                "metric_name",
                "metric_value",
    ...
    def _build_daily_label_metric_rows(
        ...
        *,
        ...
        metrics_df = (
            source_df
            .select("_model_landscape_profile_date", prediction.alias("_model_landscape_prediction"), label.alias("_model_landscape_label"))
            .filter(F.col("_model_landscape_prediction").isNotNull() & F.col("_model_landscape_label").isNotNull())
            .withColumn("_model_landscape_pred_binary", F.col("_model_landscape_prediction").cast("int"))
            .withColumn("_model_landscape_truth_binary", F.col("_model_landscape_label").cast("int"))
            .groupBy("_model_landscape_profile_date")
            .agg(
                F.sum(F.when(F.col("_model_landscape_truth_binary") == 1, F.lit(1)).otherwise(F.lit(0))).alias("actual_positive_count"),
                F.sum(F.when(F.col("_model_landscape_truth_binary") == 0, F.lit(1)).otherwise(F.lit(0))).alias("actual_negative_count"),
                F.sum(F.when(F.col("_model_landscape_pred_binary") == 1, F.lit(1)).otherwise(F.lit(0))).alias("predicted_positive_count"),
                F.sum(F.when(F.col("_model_landscape_pred_binary") == 0, F.lit(1)).otherwise(F.lit(0))).alias("predicted_negative_count"),
                F.sum(F.when((F.col("_model_landscape_pred_binary") == 1) & (F.col("_model_landscape_truth_binary") == 1), F.lit(1)).otherwise(F.lit(0))).alias("tp"),
                F.sum(F.when((F.col("_model_landscape_pred_binary") == 1) & (F.col("_model_landscape_truth_binary") == 0), F.lit(1)).otherwise(F.lit(0))).alias("fp"),
                F.sum(F.when((F.col("_model_landscape_pred_binary") == 0) & (F.col("_model_landscape_truth_binary") == 1), F.lit(1)).otherwise(F.lit(0))).alias("fn"),
                F.sum(F.when((F.col("_model_landscape_pred_binary") == 0) & (F.col("_model_landscape_truth_binary") == 0), F.lit(1)).otherwise(F.lit(0))).alias("tn"),
            )
            .withColumn(
                "precision",
    ...
    def _bin_label_expr(self, edges: tuple[float, ...], *, bin_col: str = "_model_landscape_bin_index"):
        mapping: list[Any] = []
    ...
    def _daily_classification_metrics_by_bin_df(
        ...
        feature_df = (
            ...
            .select(
                ...
                _spark_binary_prediction_column(source_df, config=config).alias("_model_landscape_prediction"),
        ...
        if not feature_df.take(1):
            return None
        bucketed = (
            _spark_bucketed_frame(feature_df, value_col="_model_landscape_feature_value", edges=edges)
            .withColumn("_model_landscape_pred_binary", F.col("_model_landscape_prediction").cast("int"))
            .withColumn("_model_landscape_truth_binary", F.col("_model_landscape_label").cast("int"))
        )
        metric_df = (
            ...
            .withColumn("_model_landscape_bin_index", F.col("_model_landscape_bin_index").cast("int"))
        )
    ...
    def _daily_regression_metrics_by_bin_df(
        ...
        feature_df = (
            ...
            .select(
                "_model_landscape_profile_date",
                _spark_col(feature).cast("double").alias("_model_landscape_feature_value"),
                _spark_col(config.contract.prediction_col).cast("double").alias("_model_landscape_prediction"),
        ...
        metric_df = (
            _spark_bucketed_frame(feature_df, value_col="_model_landscape_feature_value", edges=edges)
            .withColumn("_model_landscape_abs_error", F.abs(F.col("_model_landscape_prediction") - F.col("_model_landscape_label")))
            .withColumn("_model_landscape_squared_error", F.pow(F.col("_model_landscape_prediction") - F.col("_model_landscape_label"), 2))
            .groupBy("_model_landscape_profile_date", "_model_landscape_bin_index")
            .agg(
                F.count("*").alias("row_count"),
