...
from model_landscape.services.thresholds import THRESHOLD_METRICS, get_thresholds, merged_thresholds
from model_landscape.ui import charts
from model_landscape.ui.components import (
    make_chart_card,
    make_empty_state,
    make_metric_card,
...
def _compute_guidance_block(
    *,
    ...
    if footprint_category in {"elevated", "high"}:
        ...
        alerts.append(
            dbc.Alert(
                "No compute footprint data yet. After a few completed refresh runs, Model Landscape will show duration and scanned-row guidance here.",
                color="secondary",
                className="py-2 mb-2",
            )
        )
...
def _feature_distribution_source_message(source: str) -> str:
    ...
    if source == "unavailable_unsafe_bounded_read":
        return "Distribution source unavailable. This repository cannot enforce a hard cap on raw source-window reads, so Model Landscape skips the fallback instead of loading an unsafe volume of rows."
...
def _render_labels_discovery(
    *,
    ...
    if duplicate_join_keys > 0 and not order_col:
        alerts.append(
            dbc.Alert(
                "Labels table contains duplicate join keys without an order column. Add an order column so Model Landscape can choose the latest label per entity.",
                color="warning",
                className="mb-3",
            )
        )
    ...
    return html.Div(
        [
            ...
            html.P(
                "Model Landscape scanned the labels table, inferred the join and label columns, and validated the join against the source table.",
    ...
    def render_onboarding_wizard(
        ...
        try:
            ...
            guidance = {
                ...
                4: (
                    (
                        "Activate the monitor. Model Landscape saves the config and triggers bootstrap immediately through the shared refresh workflow."
                        if readiness_mode == "fully_ready"
                        else (
                            "Activate the monitor. Model Landscape saves the config, marks bootstrap pending, and the scheduled shared refresh workflow picks it up."
                            if readiness_mode == "scheduler_only"
            ...
            status = html.Div(
                [
                    html.Small(
                        ...
                        className="text-muted d-block model-landscape-sidebar-description",
                    ),
                    html.Small(
