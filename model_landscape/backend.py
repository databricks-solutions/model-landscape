...
class DashboardBackend:
        ...
        *,
        ...
        frame = self._warehouse.query_params(
            ...
            deduped_windows AS (
                ...
                WHERE model_landscape_window_rank = 1
            ),
        ...
        *,
        ...
        frame = self._warehouse.query_params(
            ...
            deduped_metrics AS (
                ...
                WHERE model_landscape_metric_rank = 1
            ),
            recent_windows AS (
                SELECT window_end
    ...
    def get_exact_performance_breakdown(
        ...
        *,
        ...
        working["_model_landscape_ts"] = pd.to_datetime(working[ts_col], errors="coerce")
        working = working[working["_model_landscape_ts"].notna()].copy()
        if working.empty:
            return {
                "rows": pd.DataFrame(),
                "contributors": pd.DataFrame(),
        ...
        baseline = working[
            working["_model_landscape_ts"].between(baseline_start, baseline_end, inclusive="both")
        ].copy()
        current = working[
            working["_model_landscape_ts"].between(window_start, window_end, inclusive="both")
        ].copy()
