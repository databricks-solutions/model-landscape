...
```mermaid
flowchart LR
  User["Operator in Browser"] --> App["Databricks App: Model Landscape"]
...
Monitor config writes are now atomic Delta `MERGE` operations keyed by `model_key`, so app-side saves no longer rely on a delete-then-insert gap. Monitor delete now follows the same fail-closed principle on Unity Catalog managed control-plane tables: Model Landscape uses one `BEGIN ATOMIC ... END` block for all monitor-scoped deletes and refuses to run a partial cleanup sequence if that atomic path fails. If the control plane is configured in `hive_metastore`, permanent delete is disabled with a clear capability message because that documented deployment mode cannot be assumed to support atomic multi-table cleanup; operators should archive those monitors instead or migrate the control plane to Unity Catalog before deleting.
...
- keep readers pinned to published generations only and prune each monitor back to the latest two published generations after a successful publish; Drift Analysis date-range queries read across retained published generations and dedupe repeated logical windows by latest `computed_at` / `created_at` so user-selected historical ranges are not accidentally limited to only the newest generation
...
- On Databricks CLI `v0.260.0`, the app resource can bind a SQL warehouse but not an app-level `database` or `job` resource. Model Landscape therefore treats Lakebase app reads as session/app-env configuration instead of Terraform-managed app resource wiring.
- Existing-app deployments have two supported wiring modes:
  - bundle-managed app resource mode, where [`resources/app.yml`](https://github.com/databricks-solutions/model-landscape/blob/main/resources/app.yml) still grants the app `CAN_USE` on the SQL warehouse while the deployed app source carries a literal `SQL_WAREHOUSE_ID`
  - manual existing-app mode, where [`notebooks/prepare_existing_app_source.py`](https://github.com/databricks-solutions/model-landscape/blob/main/notebooks/prepare_existing_app_source.py) generates an alternate `app.yaml` with a literal `SQL_WAREHOUSE_ID` so constrained operators do not need permission to manage app resources
