...
Monitor config writes are now atomic Delta `MERGE` operations keyed by `model_key`, so app-side saves no longer rely on a delete-then-insert gap. Monitor delete now follows the same fail-closed principle on Unity Catalog managed control-plane tables: Model Lens uses one `BEGIN ATOMIC ... END` block for all monitor-scoped deletes and refuses to run a partial cleanup sequence if that atomic path fails. If the control plane is configured in `hive_metastore`, permanent delete is disabled with a clear capability message because that documented deployment mode cannot be assumed to support atomic multi-table cleanup; operators should archive those monitors instead or migrate the control plane to Unity Catalog before deleting.
