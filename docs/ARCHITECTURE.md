...
- Existing-app deployments have two supported wiring modes:
  ...
  - manual existing-app mode, where [prepare_existing_app_source.py](../notebooks/prepare_existing_app_source.py) generates an alternate `app.yaml` with a literal `SQL_WAREHOUSE_ID` so constrained operators do not need permission to manage app resources
