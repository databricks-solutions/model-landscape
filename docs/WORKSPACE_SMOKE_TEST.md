...
This guide is the pre-client validation path for Model Landscape in your own Databricks workspace.
...
- a chosen Unity Catalog catalog/schema for the Model Landscape control plane
...
```bash
databricks workspace delete /Workspace/Users/<your-email>/.bundle/model-landscape --recursive
```

The commands below assume the default bundle variable `app_name=model-landscape`.
...
cd <repo-root>
python3 -m pytest
python3 notebooks/model_landscape_setup.py --help
python3 notebooks/model_landscape_refresh.py --help
...
```sql
SELECT COUNT(*) AS inference_rows FROM main.model_landscape_demo.inference_logs;
SELECT COUNT(*) AS label_rows FROM main.model_landscape_demo.labels;
SELECT MIN(DATE(event_ts)) AS min_date, MAX(DATE(event_ts)) AS max_date
FROM main.model_landscape_demo.inference_logs;
```
...
databricks apps get model-landscape
databricks apps get model-landscape -o json
```

Expected result:

- bundle deploy succeeds
- the Databricks app `model-landscape` exists
- `databricks apps start model-landscape` reaches `ACTIVE`
- the workflow `<app-name>-refresh` exists on Spark job compute, where `<app-name>` is `model-landscape` unless you overrode `app_name`
- the app service principal shown in `databricks apps get model-landscape -o json` still has `CAN_USE` on the SQL warehouse
- the same app identity has `CAN MANAGE RUN` on the refresh workflow

If you are reusing an existing app and cannot manage its SQL warehouse app resource, use the [Existing App Deployment](./EXISTING_APP_DEPLOYMENT.md) guide instead of this bundle-created-app path.

## Step 4: Open The App

Open the `model-landscape` Databricks App.

Check immediately:

- the app title reads `Model Landscape`
...
WHERE table_schema = 'model_landscape_ui'
ORDER BY table_name;
```
...
1. Continue to the `Discover` step and set the source table to:
   - `main.model_landscape_demo.inference_logs`
2. Optional draft inputs:
   - `Optional Labels Table`: `main.model_landscape_demo.labels`
...
6. Open `Advanced mappings and overrides` and confirm:
   ...
   If your real tables join on the same shared key name in both tables, `Entity ID Column` can stay blank and Model Landscape should still validate the join against that shared column.
...
- lifecycle actions should now be truthful: trying to archive/restore/delete a nonexistent monitor should show a warning instead of a fake success, and deleting a real monitor should either remove all monitor-scoped facts atomically or fail without partial cleanup. If the control plane uses `hive_metastore`, permanent delete should show a Unity Catalog capability warning and leave the monitor untouched; use archive/restore in that deployment mode.
...
- Drift chart titles should include the active granularity and any active filters, threshold guides should stay hidden until you enable `Show Threshold Guides`, and very small drift values should switch to scientific notation instead of collapsing into unreadable `0.0000` labels. A custom Drift date range should show all retained published windows inside that range, not just the latest refresh batch within that month.
...
- the Performance page should now expose `Per-Bin Breakdown Controls` (`Binning Mode`, `Bin Count`, `Custom Bin Edges`, and outlier controls). Auto and Fixed Bin Count should update the all-feature per-bin impact chart; Custom Bin Edges should apply only to the selected feature because one edge list is not valid across all feature scales.
...
- remaining incident readback work is tracked in Architecture docs
...
```sql
SELECT * FROM model_landscape_ui.monitor_inventory ORDER BY display_name;
SELECT * FROM model_landscape_ui.monitor_summary ORDER BY display_name;
SELECT * FROM model_landscape_ui.open_incidents ORDER BY observed_at DESC;
```
