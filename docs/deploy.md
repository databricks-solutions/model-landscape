...
- optional privileges to create:
  - the control-plane catalog if you want Model Landscape to create it from the UI
...
- the app can accelerate onboarding refreshes asynchronously by resolving `REFRESH_JOB_ID` first, then falling back to `REFRESH_JOB_NAME` (default `model-landscape-refresh`)
...
- when `REFRESH_JOB_NAME` is used, the resolver now falls back to full-workspace exact, suffix, and substring matching, so Databricks Asset Bundles development job names such as `[dev user] model-landscape-refresh` still resolve reliably
...
Hive Metastore remains supported for setup, refresh, and archive/restore workflows, but permanent monitor deletion is disabled there because Model Landscape cannot assume atomic multi-table cleanup. Use Unity Catalog managed control-plane tables if operators need `Delete Monitor And History`.
...
For `dev` or `prod`, Model Landscape expects:
...
databricks apps deploy model-landscape \
  --source-code-path /Workspace/Users/<your-email>/.bundle/model-landscape/dev/files
...
- the app `model-landscape` is created
- the workflow `<app-name>-refresh` is created on Spark job compute, where `<app-name>` is `model-landscape` unless you overrode `app_name`
- the workflow is scheduled hourly by default and acts as the shared pickup path for newly saved monitors
- `databricks apps start model-landscape` brings the app compute into `ACTIVE`
...
- `CAN_USE` on the SQL warehouse used by Model Landscape
- read access to the source data catalog/schema/tables
- read/write access to the control-plane catalog/schema/tables
- if Model Landscape should create the control-plane tables itself, `CREATE TABLE` in the control-plane schema

Treat the warehouse grant as a post-deploy check, not a one-time assumption. After every `databricks apps start model-landscape` + `databricks apps deploy model-landscape ...` cycle, verify the same app identity still has `CAN_USE` on the configured SQL warehouse and regrant it if the app shows warehouse-access errors.
...
- leave `Create catalog if missing` off unless you are using an admin identity and intentionally want Model Landscape to create the catalog
...
5. Open `Advanced mappings and overrides` and confirm:
   ...
   If the same join column name exists in both inference and labels tables, `Entity ID Column` can be left blank and Model Landscape will reuse the shared join column on the inference side.
...
6. In the same `Advanced` section, confirm features:
   ...
   - `velocity_7d`
   - `device_score`
   Model Landscape now keeps the full detected numeric feature set on by default. If your real table has dozens of numeric features, the first refresh may still take longer, but the app should not silently trim the contract.
...
- App compute stopped:
  - `databricks apps get model-landscape` shows stopped or unavailable compute; run `databricks apps start model-landscape`
- App resource exists but source is not deployed:
  - the app reports that source code has not been deployed yet; rerun `databricks apps deploy model-landscape --source-code-path ...`
