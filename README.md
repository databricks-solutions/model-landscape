...
- let the Performance page choose the drift metric, explicitly select the tracked drift features, label the lower drift chart as all tracked vs selected features, optionally overlay drift thresholds on that lower drift chart, and explain feature-impact bars and the latest-bin table in-product with “Weighted Contribution” / “Current Window Share (%)” wording instead of the older ambiguous labels
