...
- drive the Performance timeline from raw persisted daily label metrics when available so undefined daily precision/recall/F1 render as gaps instead of looking like smoothed window aggregates, while temporarily falling back to persisted weighted daily/window performance history if an upgraded workspace has not backfilled `daily_label_metrics` yet
