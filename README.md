...
- make lifecycle actions truthful and safe: archive/restore now warn when a monitor key does not exist, and permanent delete runs as one atomic warehouse transaction on Unity Catalog managed control-plane tables; `hive_metastore` deployments are told to archive instead of attempting an unsafe partial cleanup
...
- keep binary-class Data Quality filters usable even when persisted filtered daily facts are missing by deriving the requested daily quality slice directly from source + labels only when Model Lens can resolve a safe bounded date range, and show specific messages for zero-match slices vs unavailable full-range filtered history
...
- let operators edit drift/null-rate warning and critical thresholds directly on the Drift page, while reusing the same persisted per-monitor threshold overrides already used by Overview, Data Quality, and incident evaluation
...
- drive the Performance timeline from exact source-derived daily label metrics when safely bounded, otherwise from persisted `daily_label_metrics`, so undefined daily precision/recall/F1 render as gaps instead of looking like smoothed window aggregates; if neither daily source is available, the page still falls back to persisted weighted comparison-window history
- let the Performance page choose the drift metric, explicitly select the tracked drift features, label the lower drift chart as all tracked vs selected features, optionally overlay drift thresholds on that lower drift chart, apply the same binning/outlier controls used by Feature Deep Dive to the per-bin impact chart, scope custom bin edges to the currently selected feature, and explain feature-impact bars and the latest-bin table in-product with “Weighted Contribution” / “Current Window Share (%)” wording instead of the older ambiguous labels
- surface exact validation feedback for invalid Drift thresholds or invalid custom bin edges instead of collapsing those cases into a generic callback failure
...
- replace the old Performance bottom bin-detail plot with a direct handoff into Feature Deep Dive for the selected feature, so configurable binning/outlier analysis lives in one place instead of two partially overlapping views
...
- the Performance page now stacks the classification-metric timeline above the selected drift-metric timeline on the same screen so operators can compare metric drops against drift spikes without tab-hopping
- the Performance page now also lets the operator choose which tracked drift features appear in that lower drift chart; small monitors default to all tracked features and larger ones default to the top 12 for the selected drift metric
- the Performance page now also re-bins the latest comparison window live for the per-bin impact chart, using the page's `Binning Mode`, `Bin Count`, `Custom Bin Edges`, and outlier controls instead of only replaying the coarse stored slice rows
...
- for binary classification monitors, the workflow also persists `daily_class_quality_profiles`, `daily_class_feature_profiles`, and `daily_label_metrics` so Drift and Data Quality can filter by class without rescanning raw source data and Performance can render raw daily scores; until `daily_label_metrics` is populated on upgraded workspaces, the Performance timeline falls back to persisted weighted daily/window performance history instead of going blank
...
- the Feature Deep Dive dimension breakdown now labels Average vs Median (P50) explicitly and shows P25 / P50 / P75 / row-count hover details so the slice summary no longer reads like an unlabeled box plot
...
- all refresh scopes now write into a new `generation_id = run_id`, publish that generation only after every affected result table has been written, and keep most readers pinned to complete published generations so neither first bootstrap nor incremental repair leaks partial rows; Drift Analysis date-range reads can span all retained published generations for that monitor and dedupe repeated logical windows to the newest row, so historical date filters are not clipped to only the latest refresh batch
...
Hive Metastore deployments can still set up, refresh, archive, and restore monitors, but permanent deletion is disabled because atomic multi-table cleanup is not guaranteed there. Use Unity Catalog managed control-plane tables when `Delete Monitor And History` must be available.
...
5. Click `Setup Control Plane`. The `Setup` step only unlocks after setup succeeds for the current namespace values. If setup fails, fix the issue and click `Setup Control Plane` again to retry. After redeploying a newer Model Lens build into an existing workspace, run `Setup Control Plane` once so additive table migrations are applied; rerunning it is safe because existing columns/tables are checked before additive migrations are issued. Setup now also backfills legacy completed `refresh_runs` rows so older bootstrap generations gain `generation_id` and `published_at`, which makes pre-generation monitors visible to the new published-generation readers instead of staying stuck as `Computing/Pending`. If the upgrade adds `daily_label_metrics` and the workspace already has labeled monitors with historical performance data, setup also queues a one-time `performance_repair` so the Performance page can backfill raw daily labeled facts on the next scheduled repair run. Normal `performance_repair` runs also bypass the unchanged-label-watermark skip while those daily labeled facts are still missing, so upgraded monitors can self-heal after setup instead of staying permanently unavailable.
