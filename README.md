...
- make lifecycle actions truthful and safe: archive/restore now warn when a monitor key does not exist, and permanent delete runs as one atomic warehouse transaction or fails closed without partial cleanup
...
- let the Performance page choose the drift metric, explicitly select the tracked drift features, label the lower drift chart as all tracked vs selected features, optionally overlay drift thresholds on that lower drift chart, apply the same binning/outlier controls used by Feature Deep Dive to the per-bin impact chart, and explain feature-impact bars and the latest-bin table in-product with “Weighted Contribution” / “Current Window Share (%)” wording instead of the older ambiguous labels
